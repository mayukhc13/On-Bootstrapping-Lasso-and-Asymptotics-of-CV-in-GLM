...
#### h2o package can be installed from https://cran.r-project.org/bin/windows/contrib/4.3/h2o_3.40.0.4.zip or from https://cran.r-project.org/src/contrib/h2o_3.40.0.4.tar.gz
#### Need to install java from https://www.oracle.com/java/technologies/downloads/#jdk23-windows
...
design1
write_xlsx(design1,path = "M://Revised First Work//Gamma Related//gamma_design_500_7_4.xlsx") # change the path according to your own directory named as "gamma_design_500_7_4.xlsx"
...
for(m in 1:simulation)
{
  ...
  CV = h2o.glm(y=1,x=2:8,training_frame = data.gamma,nfolds = 10,standardize = FALSE,family = "gamma",link = "log",alpha = 1,lambda=lambda_sequence)
 ...
 stat3= sqrt(n)*(beta_hat_final3-beta) #centered distribution of (lasso estimate-original parameter)*sqrt(n)
  ...
  stat4
  stat3_norm = norm(stat4,type = "f") # euclidean norm of above to check count norm CI 
  stat3_norm
...
g_inv_der = vector()
  ...
  for(c in 1:B)
  {
    ...
    penalty_boot = (p_norm(v2,1))*lambda_opt
